        ...
        <div class="row ">
            ...
            <div class="col-md-6 col-lg-3 mb-2">
                <select class="form-select" name="status">
                    ...
                    <option value="confirm" {{ Request()->status == 'confirm' ? 'selected' : '' }}>Xác nhận
                    </option>
                    <option value="completed" {{ Request()->status == 'completed' ? 'selected' : '' }}>Hoàn thành
                    </option>
                    <option value="refunded" {{ Request()->status == 'refunded' ? 'selected' : '' }}>Trả hàng
                    </option>
                </select>
            </div>
        ...
        <table class="table">
            ...
            <tbody class="table-border-bottom-0">
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ route('dashboard.order.order-detail', $item->id) }}" style="color: inherit    "
                    ...
                    <td class="text-center px-0">
                        ...
                        <select onchange="changeStatusOrder(this,{{ $item->id }})" class="form-select status__order ">
                            <option value="pending" @if ($item->status == 1) @selected(true) @endif>
                                Chờ xác nhận</option>
                            <option value="confirm" @if ($item->status == 2) @selected(true) @endif>
                                Xác nhận
                            </option>
                            <option value="completed" @if ($item->status == 3) @selected(true) @endif>
                                Hoàn thành
                            </option>
                            <option value="refunded" @if ($item->status == 4) @selected(true) @endif>
                                Trả hàng
                            </option>
                        </select>
                    </td>
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item"
                                    ...
                                    Xem thêm</a>
                                <form class="dropdown-item" action="{{ route('dashboard.order.delete', $item->id) }}"
                                    method="POST" onsubmit="return confirm('Bạn chắc chắn muốn xóa vĩnh viễn?')">
                                    @csrf
                                    @method('delete')
