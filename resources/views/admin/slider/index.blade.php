...
@section('content')
    ...
    <div class="card">
        <x-alert />
        <x-header-table tableName="Danh sách {{ $moduleName }}" link="dashboard.slider.add"
                        ...
                        @foreach ($sliders as $item)
                            <tr>
                                <td class="px-0 text-center">
                                    <a href="{{ route('dashboard.slider.edit', $item->id) }}" title="Click xem thêm"
                                        style="color: inherit"><strong>{{ $item->id }}</strong>
                                    </a>
                                </td>
                                <td class="px-0 text-center">
                                    <img src="{{ $item->image ?? asset('images/no-img.png') }}" style="object-fit: contain"
                                        alt="Ảnh" class=" border rounded w-px-100 h-px-100">
                                </td>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item"
                                                href="{{ route('dashboard.slider.edit', $item->id) }}"><i
                                            ...
                                            @if ($item->trashed() == 1)
                                                <form class="dropdown-item"
                                                    action="{{ route('dashboard.slider.restore', $item->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('delete')
                                            ...
                                            @endif
                                            <form class="dropdown-item"
                                                action="{{ $item->trashed() ? route('dashboard.slider.force-delete', $item->id) : route('dashboard.slider.soft-delete', $item->id) }}"
                                                method="POST"
